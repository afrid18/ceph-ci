...
int D3NFilterDriver::get_bucket(const DoutPrefixProvider* dpp, User* u, const rgw_bucket& b, std::unique_ptr<Bucket>* bucket, optional_yield y)
{
  std::unique_ptr<Bucket> bp;
  int ret = next->get_bucket(dpp, u, b, &bp, y);
  if (ret < 0) {
    return ret;
  }

  Bucket* d3n_bp = new D3NFilterBucket(this, std::move(bp), u);
  bucket->reset(d3n_bp);
  return 0;
}

int D3NFilterDriver::get_bucket(User* u, const RGWBucketInfo& i, std::unique_ptr<Bucket>* bucket)
{
  std::unique_ptr<Bucket> bp;
  int ret = next->get_bucket(u, i, &bp);
  if (ret < 0) {
    return ret;
  }
...
int D3NFilterDriver::get_bucket(const DoutPrefixProvider* dpp, User* u, const std::string& tenant, const std::string& name, std::unique_ptr<Bucket>* bucket, optional_yield y)
{
  rgw_bucket b;
			...
			RGWGetDataCB* cb, optional_yield y)
{
  ...
  do {
    ...
    if (filter->get_d3n_cache()->get(oid_in_cache, part_len)) {
      // Read From Cache
      auto completed = aio->get(r_obj, rgw::Aio::d3n_cache_op(dpp, y, read_ofs, len_to_read, filter->get_d3n_cache()->cache_location), cost, id);
