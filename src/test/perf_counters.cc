...
void configure_counters_expiration(PerfCounters* counters, bool time_filtered, uint64_t time) {
  counters->time_filtered = time_filtered;
...
TEST(PerfCounters, TestLabeledCountersDumpFilteredByTime1Counter) {
  ...
  configure_counters_expiration(counters1, true, 2);
  ...
  // TESTCASE: counter dumped again after new expiration time
  configure_counters_expiration(counters1, true, 2);
  this_thread::sleep_for(1s);
  ASSERT_EQ("", client.do_request(R"({ "prefix": "counter dump", "format": "raw" })", &message));
  ASSERT_EQ(R"({
  ...
  // TESTCASE: counter dumped after time_filtered config variable changed
  configure_counters_expiration(counters1, false, 10);
  ASSERT_EQ("", client.do_request(R"({ "prefix": "counter dump", "format": "raw" })", &message));
  ASSERT_EQ(R"({
    "ctrs": [
        {
  ...
  // TESTCASE: counter filtered after time_filtered config variable changed
  configure_counters_expiration(counters1, true, 0);
  this_thread::sleep_for(2s);
  ASSERT_EQ("", client.do_request(R"({ "prefix": "counter dump", "format": "raw" })", &message));
  ASSERT_EQ("{}\n", message);
...
TEST(PerfCounters, TestLabeledCountersDumpFilteredByTime2Counters) {
  ...
  configure_counters_expiration(counters1, true, 1);
  configure_counters_expiration(counters2, true, 8);
  this_thread::sleep_for(2s);
  ASSERT_EQ("", client.do_request(R"({ "prefix": "counter dump", "format": "raw" })", &message));
  ASSERT_EQ(R"({
    "ctrs": [
  ...
  configure_counters_expiration(counters1, true, 8);
  configure_counters_expiration(counters2, true, 1);
  this_thread::sleep_for(2s);
  ASSERT_EQ("", client.do_request(R"({ "prefix": "counter dump", "format": "raw" })", &message));
  ASSERT_EQ(R"({
    "ctrs": [
  ...
  configure_counters_expiration(counters1, false, 1);
  configure_counters_expiration(counters2, false, 1);
  ASSERT_EQ("", client.do_request(R"({ "prefix": "counter dump", "format": "raw" })", &message));
  //this_thread::sleep_for(1s);
  ASSERT_EQ(R"({
    "ctrs": [
...
TEST(PerfCounters, TestLabeledCountersDumpFilteredByTime6Counters) {
  ...
  configure_counters_expiration(counters2, true, 1);
  configure_counters_expiration(counters4, true, 1);
  configure_counters_expiration(counters6, true, 1);
  this_thread::sleep_for(2s);
  ASSERT_EQ("", client.do_request(R"({ "prefix": "counter dump", "format": "raw" })", &message));
  ASSERT_EQ(R"({
    "ctrs": [
  ...
  configure_counters_expiration(counters1, true, 1);
  configure_counters_expiration(counters2, true, 10);
  configure_counters_expiration(counters3, true, 1);
  configure_counters_expiration(counters4, true, 10);
  configure_counters_expiration(counters5, true, 1);
  configure_counters_expiration(counters6, true, 10);
  this_thread::sleep_for(2s);
  ASSERT_EQ("", client.do_request(R"({ "prefix": "counter dump", "format": "raw" })", &message));
  ASSERT_EQ(R"({
    "ctrs": [
  ...
  configure_counters_expiration(counters3, true, 1);
  configure_counters_expiration(counters4, true, 10);
  configure_counters_expiration(counters5, true, 10);
  configure_counters_expiration(counters6, true, 10);
  this_thread::sleep_for(2s);
  ASSERT_EQ("", client.do_request(R"({ "prefix": "counter dump", "format": "raw" })", &message));
  ASSERT_EQ(R"({
  ...
  configure_counters_expiration(counters1, true, 10);
  configure_counters_expiration(counters2, true, 10);
  configure_counters_expiration(counters3, true, 10);
  configure_counters_expiration(counters4, true, 1);
  configure_counters_expiration(counters5, true, 1);
  configure_counters_expiration(counters6, true, 1);
  this_thread::sleep_for(2s);
  ...
  configure_counters_expiration(counters1, false, 1);
  configure_counters_expiration(counters2, false, 1);
  configure_counters_expiration(counters3, false, 1);
  configure_counters_expiration(counters4, false, 1);
  configure_counters_expiration(counters5, false, 1);
  configure_counters_expiration(counters6, false, 1);
  ASSERT_EQ("", client.do_request(R"({ "prefix": "counter dump", "format": "raw" })", &message));
  ASSERT_EQ(R"({
    "ctrs": [
        {
